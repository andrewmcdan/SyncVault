...
async function handleLocalChange(destinationPath: string): Promise<void> {
  publishStatus({
    state: "syncing",
    message: `Syncing local change: ${path.basename(destinationPath)}`,
  ...
  if (!context.local_clone_path || !context.mapping_path || !context.template_path) {
    publishStatus({
      state: "ready",
      message: "Watching for changes",
      updatedAt: new Date().toISOString()
  ...
  try {
    ...
    appendLog(
      createLog(
        "error",
        `Local sync failed for ${destinationPath}: ${error?.message ?? "unknown error"}`
      )
    );
  }
}
